...
|-------------------|-----------------------------------------------------------------------------------------------------------------------------------------------------------------|------------------------------------|
| `off`             | `coderd_chatd_stage_metrics_level` only                                                                                                                         | none                               |
| `basic` (default) | `stage_duration_seconds` (12 buckets), `turn_time_seconds_total`, `turns_total`, `stage_anomalies_total`, `stage_metrics_level`                                 | the `[basic]` stages above         |
| `full`            | basic with 16 buckets on `stage_duration_seconds`, plus `turn_time_seconds`, `turn_time_share`, `turn_stage_seconds`, `turn_stage_count`, `stage_share_of_turn` | all 15                             |
...
Series cost per `(chat_kind, model)` pair is roughly 90 at `basic`
(plus a constant ~180 per replica for the model-less wait and connect
stages) and roughly 1,200 at `full`. Multiply by replicas and by the
number of models in use. Almost all of the `basic` cost is
`stage_duration_seconds`; the turn partition is two counters per
category and model.

### Alerting without the dashboard

Alerts only need `coderd_chatd_stage_duration_seconds` for the stages in
question. The buckets are round numbers (`0.1 0.25 0.5 1 2.5 5 10 30 60
...
metric_relabel_configs:
  ...
  - source_labels: [__name__]
    regex: 'coderd_chatd_(turns?_.*|stage_share_of_turn)'
    action: drop
```
...
partition row (`coderd_chatd_turn_time_seconds_total`,
`coderd_chatd_turns_total`, `coderd_chatd_turn_stage_seconds`,
