...
func TestTurnAccountingStageTotals(t *testing.T) {
	...
	require.NotContains(t, labels, "effort", "effort is a span attribute, not a metric label")
...
func TestTurnMetricsLevels(t *testing.T) {
	...
	turnFamilies := []string{
		"coderd_chatd_stage_share_of_turn",
		"coderd_chatd_turn_stage_count",
	...
	for _, tt := range tests {
		t.Run(string(tt.level), func(t *testing.T) {
			...
			metrics.RecordTurnStage(StageStream, ChatKindRoot, "m", time.Minute, 0.5, 2)
			metrics.RecordTurn(ChatKindRoot, "m")
			metrics.RecordTurnCategory(CategoryStreaming, ChatKindRoot, "m", time.Minute, 0.5)
			metrics.RecordTurnCategory(CategoryScheduling, ChatKindRoot, "m", 0, 0)
